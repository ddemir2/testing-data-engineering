...
for index, vehicle_num in enumerate(dazzle_list):
  try:
    print(f'\nCalling API for vehicle #{vehicle_num} . . . ({index+1}/{all_vehicles})')
    ...
    for h1 in h1_tags:
      ...
      if find_date:
        service_date = find_date.group(1) # Keep as string for JSON serialization
            ...
            try:
              ...
              if len(cells) == 24:
                record = {
                  ...
                  "trip_number" : str(attempted_match.group(1)),
    ...
    print(f'\t!!! Error for vehicle #{vehicle_num}: {e}')
...
try:
  ...
  print(f"Failed to send sentinel: {sent_error}")

#--Summary Stats---------------------------------------------------------
final_time = time.time()
elapsed_time = final_time - start_time
throughput = float(breadcrumb_count) / float(elapsed_time) if elapsed_time > 0 else 0
received_vehicles = all_vehicles - len(failed_requests)

print(f'\n\nRecords published: {breadcrumb_count}')
#print(f'Vehicles with no data: {failed_requests}')
print(f'Vehicles with available records: {received_vehicles}')
print(f'Start time: {time.ctime(start_time)}')
print(f'End time: {time.ctime(final_time)}')
